# -*- coding: utf-8 -*-
#!/usr/bin/env python

#카메라 선언부
from __future__ import print_function

try:
	import cv2
	from ar_markers import detect_markers
except ImportError:
	raise Exception('Error: OpenCv is not installed')


#Serial lib
import serial

#Bluetooth lib
from bluetooth import *

#GPIO lib
import RPi.GPIO as GPIO
from time import sleep

#모터 상태
STOP = 0
FORWARD = 1
BACKWARD = 2

# PIN설정
HIGH = 1
LOW = 0

# Create the client socket 
BT_socket = BluetoothSocket( RFCOMM )    
BT_socket.connect(("98:D3:81:FD:6E:85", 1)) #소켓통신 연결


#실제 핀 정의
PWM_DC = 18 #BCM

#GPIO pin
DC_Motor_A = 20
DC_Motor_B = 21

#서보모터 PWM핀
B_SERVO = 23

#시리얼 통신
SER = serial.Serial('/dev/ttyACM0',baudrate = 9600, timeout = None)

#GPIO 모드설정
GPIO.setmode(GPIO.BCM)
GPIO.setwarnings(False)


# 핀 설정 함수

#모터 핀 설정
def setPinConfig(EN, INA, INB):

    GPIO.setup(EN, GPIO.OUT)
    GPIO.setup(INA, GPIO.OUT)
    GPIO.setup(INB, GPIO.OUT)
    # 100khz 로 pwm 제어
    pwm = GPIO.PWM(EN, 100)
    # 우선 pwm 정지
    pwm.start(0)

    return pwm


#SERVO 핀 및 PWM 설정

GPIO.setup(B_SERVO, GPIO.OUT)
S_pwm = GPIO.PWM(B_SERVO, 50)
S_pwm.start(0)


# 모터제어함수
def setMotorControl(pwm, INA, INB, speed, state):

     #모터 속도 제어 PWM
    pwm.ChangeDutyCycle(speed)

    #전진
    if state == FORWARD:
        GPIO.output(INA, HIGH)    
        GPIO.output(INB, LOW)
    #후진
    if state == BACKWARD:
        GPIO.output(INA, LOW)    
        GPIO.output(INB, HIGH)
    #정지
    if state == STOP:
        GPIO.output(INA, LOW)    
        GPIO.output(INB, LOW)

#모터제어함수를 간단히 사용하기위해 한번더 래핑
def setMotor(ch, speed, state):
    if ch == 'DC':
        setMotorControl(pwmA, DC_Motor_A, DC_Motor_B, speed, state)

#핀 설정후 PWM 얻어옴
pwmA = setPinConfig(PWM_DC, DC_Motor_A, DC_Motor_B)

#서보제어함수
def setServo (Spwm):
    S_pwm.ChangeDutyCycle(Spwm)
    



 

#제어 부분

if __name__ == '__main__':
	print('Press "q" to quit')
	capture = cv2.VideoCapture(0)

	if capture.isOpened():  # try to get the first frame
		frame_captured, frame = capture.read()
	else:
		frame_captured = False

        setMotor('DC', 30, FORWARD)
	setServo(7) #정방향
	error_cnt = 0
	marker_cnt = 0

	while frame_captured:

		markers = detect_markers(frame)
          
                ultra_data = SER.readline() #초음파 거리 수신
		
                ultra_data = long(ultra_data) #수신된 값을 변환
		


		print("distance=",ultra_data) #거리값 출력
		ultra_data = str(ultra_data) +'@'
		if markers:
			marker_cnt = 1
			error_cnt = 0
			for marker in markers:
				error_cnt = 0	
				marker.highlite_marker(frame)
				#print ('Marker ID:',marker.id)
				#print('Marker center:',marker.center)
				x = marker.center[0]
				y = marker.center[1]
		
				if(marker.id != 1): #찾던마커가 아니면 계속 주행 
					setMotor('DC', 50, FORWARD)
					setServo(7) #정방향
				elif(marker.id == 1): #마커를 찾을경우 나중에 정지 속도 조정 
					#print(marker)
					
	
					if( 265 <= x and x <=365 ):
						#if(175 <= y and y <= 275): y축은 높이이므로 무시
						setMotor('DC', 0, STOP)
						setServo(7) #정방향
						SER.write('S')
						BT_socket.sendall(ultra_data) #string만 보냄.
					
					
					elif( x < 265 ):
						setMotor('DC', 30, BACKWARD)
						setServo(7) #정방향
					else:
						setMotor('DC', 30, FORWARD)
						setServo(7) #정방향


		else:
			error_cnt = 1;

			if marker_cnt == 1 and error_cnt == 1:
				setMotor('DC', 0, STOP)
				setServo(7) #정방향
				print('error!No marker, STOP') 
				#BT_socket.send(ultra_data)
				#SER.write('1')
				marker_cnt = 0



		cv2.imshow('Test Frame', frame)
		if cv2.waitKey(1) & 0xFF == ord('q'):
			break
		#print (markers)
		frame_captured, frame = capture.read()
		#print markers
	# When everything done, release the capture
	capture.release()
	cv2.destroyAllWindows()

       

